...
dbutils.widgets.combobox(
    "batch_wait", "30", ["15", "30", "45", "60"], "Speed (secs between writes)"
)
dbutils.widgets.combobox(
    "num_recs", "10000", ["5000", "10000", "20000"], "Volume (# records per writes)"
)
dbutils.widgets.combobox(
    "batch_count",
    "100",
    ["100", "200", "500"],
    "Write count (for how long we append data)",
)
...
).write.format("delta").save(output_path + "/ref_accounting_treatment")

dbutils.fs.put(
    output_path + "/account_schema.json",
    """{
...
def generate_loans(fire_schema, n=100):
    ...
    f = open(
        "/dbfs" + output_path + "/account_schema.json", "r"
    )  # output_path + "/" + fire_schema + ".json"
    schema = json.load(f)
...
for i in range(0, int(dbutils.widgets.get("batch_count"))):
    ...
    write_batches_to_file(
        generate_loans("account", int(dbutils.widgets.get("num_recs"))),
        f"/dbfs{output_path}/landing/accounts{i}.json",
    )
